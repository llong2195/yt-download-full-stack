...
function disableExtractor(): void {
  ...
  document.querySelectorAll("[data-ytb-processed]").forEach((item) => {
    const element = item as ExtendedHTMLElement;
    if (element._ytbClickBlocker) {
      element.removeEventListener("click", element._ytbClickBlocker, true);
      delete element._ytbClickBlocker;
    }
    delete element.dataset.ytbProcessed;
    delete element.dataset.ytbVideoId;
  });
...
function processPage(): void {
  ...
  items.forEach((item) => {
    ...
    let injectionTarget: HTMLElement | null = null;
    let isShorts = false;
    ...
    if (stdThumbnail) {
      const anchor =
        stdThumbnail.querySelector<HTMLAnchorElement>("a#thumbnail");
      if (anchor && anchor.href) {
        ...
        if (extractedId) {
          ...
          const detailsArea = item.querySelector<HTMLElement>("#details");
          if (detailsArea) {
    ...
    if (!item._ytbClickBlocker) {
      item._ytbClickBlocker = function (e: Event): boolean {
        // Block navigation
        e.preventDefault();
        e.stopPropagation();
...
function extractMetadata(id: string, parentItem: HTMLElement): VideoData {
  ...
  // --- Title Extraction ---
  // 1. Standard
  const titleEl = parentItem.querySelector<HTMLElement>("#video-title");
  if (titleEl) {
    title = titleEl.textContent?.trim() || title;
  } else {
    const titleLink =
      parentItem.querySelector<HTMLAnchorElement>("a#video-title-link");
    if (titleLink)
      title = titleLink.title || titleLink.textContent?.trim() || title;
  }
  ...
  let channelLink =
    parentItem.querySelector<HTMLAnchorElement>("ytd-channel-name a");
...
chrome.runtime.onMessage.addListener(
  (msg: Message, sender, sendResponse: (response?: any) => void) => {
    if (msg.action === "toggleExtractor") {
      ...
      document
        .querySelectorAll<HTMLElement>(".yt-extractor-checkbox.selected")
        .forEach((el) => {
